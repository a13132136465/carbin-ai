...
class ProjectInput(BaseModel):
    ...
    annual_generation_unit: Optional[str] = Field(
        default=None,
        description=(
            "Unit explicitly provided by the user for annual electricity "
            "generation, such as MWh or kWh. "
            "Do not infer or guess the unit. "
            "None if the user does not explicitly provide a unit."
        ),
    )
    ...
    grid_emission_factor_value: Optional[float] = Field(
        default=None,
        description=(
            "Numeric value of the grid emission factor. " "None if not provided."
        ),
    )
    ...
    grid_emission_factor_unit: Optional[str] = Field(
        default=None,
        description=(
            "Unit explicitly provided by the user for the grid emission "
            "factor, such as tCO2/MWh. "
            "Do not infer or guess the unit. "
...
def parse_project(state: CarbonAuditState):
    ...
    if project.annual_generation_mwh is not None:
        updates["annual_generation_mwh"] = project.annual_generation_mwh

    if project.grid_emission_factor is not None:
        updates["grid_emission_factor"] = project.grid_emission_factor
...
def validate_project(state: CarbonAuditState):
    ...
    if state["annual_generation_mwh"] is None:
        missing_fields.append("annual_generation_mwh")

    if state["grid_emission_factor"] is None:
        missing_fields.append("grid_emission_factor")
    ...
    if state["annual_generation_mwh"] <= 0:
        return {
    ...
    if state["grid_emission_factor"] <= 0:
        return {
            "validation_status": "INVALID",
...
builder.add_conditional_edges(
    ...
    {"calculate": "calculate", "ask_for_info": "ask_for_info", "reject": "reject"},
)
